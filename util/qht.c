...
void qht_htable_traverse(const struct qht *ht, const void *userp, qht_lookup_func_t func_cmp_tb, qht_invalidate_func_t func_invalidate_tb)
{
    const struct qht_bucket *b;
    const struct qht_map *map;
    int i, j;
    ...
    for(i = 0; i < map->n_buckets; i++)
    {
        ...
        do {
            for (j = 0; j < QHT_BUCKET_ENTRIES; j++) {
                if (qatomic_read(&b->hashes[i])) {
                    void *p = qatomic_rcu_read(&b->pointers[i]);

                    if (likely(p) && likely(func_cmp_tb(p, userp))) {
                        func_invalidate_tb(p);
                    }
                }
            }
            b = qatomic_rcu_read(&b->next);
