...
function check_key_1()
{
	...
	for(var i = 1; i < 2; i++){
		...
		if (wep_def_key[i-1].selectedIndex){
			...
			if (key.length != wep_key_len && key.length != hex_len){
				alert(LangMap.which_lang['TEXT041_1']+" " + [i+4] + " "+LangMap.which_lang['TEXT041_2']+" " + wep_key_len + " "+LangMap.which_lang['TEXT041_3']+" " + hex_len + " "+LangMap.which_lang['TEXT041_4']);//TEXT041			    		
				return false;
			}else if(key.length == hex_len){
				for (var j = 0; j < key.length; j++){
					if (!check_hex(key.substring(j, j+1))){
					alert(LangMap.which_lang['TEXT042_1']+" " + [i+4] + " "+LangMap.which_lang['TEXT042_2']);//TEXT042			      			
						return false;
					}
				}
				if(i == def_key)
...
function check_pwd(pwd1, pwd2){
	if (get_by_id(pwd1).value != get_by_id(pwd2).value){
		 alert(LangMap.which_lang['YM102']);
		return false;
	}
	return true;
}
...
function check_ssid(id){
	if (get_by_id(id).value == ""){
	    alert(LangMap.msg['SSID_EMPTY_ERROR']);
	    return false;
	}
	return true;
}
...
function copy_ip(index){
	...
	if (get_by_id("ip_list" + index).selectedIndex > 0){
		...
		alert(LangMap.which_lang['SELECT_COMPUTER_ERROR']);
	}
}
