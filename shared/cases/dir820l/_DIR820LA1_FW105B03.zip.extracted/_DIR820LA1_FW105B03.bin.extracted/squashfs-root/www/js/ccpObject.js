...
function ccpObject(ccp,act) {
	...
	// Dlink need to set GUI timeout
	var gui_timeout = true;
	...
	this.get_router_info = function() {
		...
		var info = {
			...
			'domain':		this.config_val('RF_Domain'),//DIR is hw domain
			...
			'KCode_USB':	this.config_val('KCode_USB'),
			'EN_SPEC_WFA_MYDLINK_1_00':	this.config_val('EN_SPEC_WFA_MYDLINK_1_00')||0
		};
	...
	this.get_host_list = function( ref ) {
		...
		if(allHostIp != null)
		{
			for (var i=0; i<allHostIp.length; i++)
			{
				...
				{
					if(allHostIp[i] != '')
						document.write('<option value='+allHostIp[i]+'>'+sp_words(allHostName[i])+'('+allHostIp[i]+')</option>');
				}
				else if(ref == 'mac')
				{
					if(allHostMac[i]!= '')
						document.write('<option value='+allHostMac[i]+'>'+sp_words(allHostName[i])+'('+allHostMac[i]+')</option>');
				}
				else
					document.write('<option value='+allHostName[i]+'>'+sp_words(allHostName[i])+'</option>');
	...
	this.set_host_list_1 = function( ref ) {
		...
		if(allHostIp != null)
		{
			for (var i=0; i<allHostIp.length; i++)
			{
				...
				{
					if(allHostIp[i] != '')
						list_str += '<option value='+allHostIp[i]+'>'+sp_words(allHostName[i])+'('+allHostIp[i]+')</option>';
				}
				else if(ref == 'mac')
				{
					if(allHostMac[i]!= '')
						list_str += '<option value='+allHostMac[i]+'>'+sp_words(allHostName[i])+'('+allHostMac[i]+')</option>';
				}
				else
					list_str += '<option value='+allHostName[i]+'>'+sp_words(allHostName[i])+'</option>';
			}
		}
