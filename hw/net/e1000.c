...
static void fdta_taint_mark(const struct iovec *iov, hwaddr ba)
{
    int http_head = 0;
    int http_len = 0;
    int taint_head = 0;
    ...
    is_url_right = strncmp((char *)url, mark_msg.url, strlen(mark_msg.url));  

    if(is_http && !is_url_right && !repeat) {
        is_taint = match_taint_data(iov->iov_base, mark_msg.label, &taint_head, &taint_len);
        if (is_taint) {
            // fresh_taint_memory();
            taint = (uint8_t *)malloc(taint_len);
            memset(taint, 0xFF, taint_len);
            taint_mem(ba + taint_head + mark_msg.taint_start, mark_msg.taint_len, taint);
            printf("TAINT MARKED!\n");
            free(taint);
            taint = NULL;
            repeat = 1;
