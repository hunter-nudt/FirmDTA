                     ...
                     CPUState *cpu, TranslationBlock *tb, int max_insns)
{
    ...
    if(fdta_is_block_begin_callback_needed(tb->pc)) {
        TCGv_ptr tmpCs = tcg_const_ptr((tcg_target_ulong)cpu);
        TCGv_ptr tmpTb = tcg_const_ptr((tcg_target_ulong)tb);
        gen_helper_fdta_invoke_block_begin_callback(tmpCs, tmpTb);
...
#if 0
    if(taint_start) {
        fdta_register_callback(FDTAF_INSN_BEFORE_CB, insn_msg, 1);
        taint_start = false;
    }
#endif
    ...
    while (true) {
        ...
        // if (fdta_is_callback_needed(FDTAF_INSN_BEGIN_CB)) {
        //     gen_helper_fdta_invoke_insn_before_callback(cpu_env);
        // }   
        db->num_insns++;
        ...
        // /* Running callback function when gen a new instruction */
        // if (fdta_is_callback_needed(FDTAF_INSN_END_CB)) {
        //     gen_helper_fdta_invoke_insn_after_callback(cpu_env);
        // }
    }
